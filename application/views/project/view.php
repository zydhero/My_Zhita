<? $this->view('header') ?>
<link href="<?=base_url()?>style/项目详细2.css" rel="stylesheet" type="text/css" />
...
with (document)
	0[(getElementsByTagName('head')[0] || body).appendChild(createElement('script')).src = 'http://bdimg.share.baidu.com/static/api/js/share.js?v=89860593.js?cdnversion=' + ~(-new Date() / 36e5)];</script>
        ...
        <div class="bg2">
            <div class="sub">
            <div class="i">目前有<?= count($wits) ?>种创意</div>
                <? if ($this->user->id == $project['company'] && !$this->user->is_muted) { ?>
                    <a href="<?=site_url()?>/company/project/<?= $project['id'] ?>" class="btn">编辑</a>
                <? } elseif ($this->user->isLogged(array('witower', 'project')) && !$this->user->is_muted) { ?>
                    <a href="<?=site_url()?>/admin/project/<?= $project['id'] ?>" class="btn">编辑</a>
                <? } else { ?>
                <form method="post" class="btn btn-primary" action="<?=site_url()?>/wit/add?project=<?= $project['id'] ?>"
                    ...
                    <? } ?>>
                    <button type="submit" value="1" id="sub">发布创意</button>
                    <? } ?>
		    ...
		    <div class="all">
                ...
                <div class="right">
                    ...
                    <a href="<?=site_url()?>/wit/versions/<?= $wit['id'] ?>" target="_blank">版本</a>
               		<? if (($this->user->isLogged(array('witower', 'wit')) || $this->user->id == $project['id']) && $project['status'] === 'buffering') { ?>
								...
								<? } ?>
							<? } ?>
							<? if ($project['status'] === 'witting' && !$this->user->is_muted) { ?><a href="<?=site_url()?>/wit/edit/<?= $wit['id'] ?>" class="btn btn-small btn-primary" style="margin-top: 2px; margin-right: 1em;">编辑</a><? } ?>
                </div>
            </div>
        <div class="bg3">
            <?= $wit['content'] ?>
							...
							<? foreach ($wit['comments'] as $comment) { ?>
								<li>
									<dl class="dl-horizontal">
										<dt>
										<a href="<?=site_url()?>/space/<?= $comment['user'] ?>"><img src="<?=base_url()?>uploads/images/avatar/<?=$comment['user']?>.jpg"></a>
										</dt>
										<dd>
											<p class="avatar">
